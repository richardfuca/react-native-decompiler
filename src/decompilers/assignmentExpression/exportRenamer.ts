...
export default class ExportRenamer extends Decompiler<AssignmentExpression> {
  actionable(path: NodePath<AssignmentExpression>): boolean {
    return path.node.operator === '=' && path.node.left?.type === 'MemberExpression' &&
      ...
      path.scope.bindings[path.node.left.object.name].identifier.start === this.module.exportsParam.start;
  }
